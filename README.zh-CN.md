...
### 4. 开 256K 上下文,长上下文下 decode 约降 26%

| `-c` | 就绪 GTT | Vulkan0 计算缓冲 | prefill | decode @52K | decode 短请求 |
|---:|---:|---:|---:|---:|---:|
| 131072 | 87.1 GiB | 2322 MiB | 208.1 | **27.18** | 27.55 |
| 262144 | 93.0 GiB | 4386 MiB | 206.8 | **20.23** | 26.87 |

能载入,内存也够。短请求几乎无影响(26.87 vs 27.55),52K 长上下文约降 26%。
两者接受率**逐位完全相同**(0.69663,310/445,平均长度 4.48),所以损失在
**目标模型**的大 `n_ctx`,不在投机侧。prefill 也不受影响(206.8 vs 208.1)。

> **测量注意事项。** 同一配置更早的一次单独测量得到 13.74 t/s —— 比后来同样设置下的
> 20.23 低 47%。这个模型的 decode **单次测量不可复现**:那次是在连续多轮服务起停之后
> 跑的。**每个配置至少测两次。** 本仓库的数字除非特别说明都是单次测量,
> 10% 以内的差异请视为未定论。
