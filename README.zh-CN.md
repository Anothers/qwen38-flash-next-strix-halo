...
> **在采信下面任何数字之前,先读 [`docs/TUNE-FOR-YOUR-WORKLOAD.md`](docs/TUNE-FOR-YOUR-WORKLOAD.md)(英文)。**
> 本文所有测量都用 52K 合成 prompt,而在写这份文档的机器上,那个长度**超出真实流量的 99 分位**。
> `--spec-draft-n-max` 的最优值在短 prompt 对话下是 **2**、代码生成下是 **3**,而不是本基准
> 给出的 **5** —— 在真正占主导的负载上差 18%。该文档同时**撤回**了「`-c 262144` 拖慢 decode」这一结论。
...
早先版本称 `-c 262144` 在长上下文下让 decode 慢 26~43%。**此结论撤回。** 支持它的扫描存在
固定顺序缺陷,顺序反转的对照实验从未成功完成,而在真实工作点上两档测得完全一致。
源码复核发现所有相关量都按**实际用量**而非分配量计算。完整说明见
[`docs/TUNE-FOR-YOUR-WORKLOAD.md`](docs/TUNE-FOR-YOUR-WORKLOAD.md)。

选较小的 `-c` 仍然合理 —— 因为它少占内存(本机 87.1 vs 93.0 GiB),只是不能用之前给的吞吐理由。
